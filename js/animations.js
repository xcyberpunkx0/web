...
class AnimationManager {
    ...
    getAnimationType(element) {
        ...
        if (element.classList.contains('timeline-item')) return 'animate-slide-left';
        if (element.classList.contains('credential-item')) return 'animate-fade-in';
        if (element.classList.contains('contact-item')) return 'animate-slide-right';
    ...
    calculateStaggerDelay(element) {
        ...
        const container = element.closest('.services-grid, .team-grid, .stats-grid, .timeline-container, .credentials-grid');
    ...
    initClickEffects() {
        ...
        const interactiveElements = document.querySelectorAll('.btn, .service-card, .team-card, .nav-link');
    ...
    const teamObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const teamImages = entry.target.querySelectorAll('.team-card-image img');
...
function enhanceTeamModalAnimations() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.team-card') && !e.target.closest('.team-contact-btn')) {
...
function initTeamParallax() {
    ...
    if (!teamSection || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        return;
    }
...
function initTouchOptimizations() {
    ...
    const touchElements = document.querySelectorAll('.service-card, .team-card, .btn, .nav-link');
...
function initMobileIntersectionObserver() {
    ...
    const animateElements = document.querySelectorAll('.service-card, .team-card, .stat-item, .faq-item');
